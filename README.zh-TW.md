...
- ✅ 支援明水印和暗水印：截圖匯出時可自動疊加透明文字水印，或寫入可辨識的頻域暗水印。
...
- 明水印：開啟後在複製、下載或固定截圖時疊加自訂透明文案，支援四角、水平平鋪和傾斜平鋪。
- 暗水印：開啟後在複製、下載或固定截圖時寫入自訂暗水印；設定頁可選擇圖片辨識暗水印內容，辨識結果過長時 hover 顯示完整內容。
...
- 水印只在最終匯出階段套用，覆蓋複製、下載和固定到螢幕；OCR、QR Code 辨識和翻譯仍使用原始選區，避免水印干擾辨識。
- 暗水印主流程使用 8x8 DCT 亮度中頻係數差分嵌入，並對 header 和正文做冗餘寫入、辨識時以多數投票恢復；payload 帶 magic、長度和校驗，同時保留舊版 LSB 讀寫作為小圖/舊圖相容兜底。
...
src/logic/watermark.ts  明水印繪製、暗水印寫入與辨識
...
- 暗水印用於輕量追蹤和辨識，不是 DRM 或防竄改機制；同尺寸 PNG/JPEG/WebP 重新編碼比舊 LSB 更穩，但大幅縮放、裁切、旋轉、強壓縮、濾鏡處理或二次截圖後仍可能辨識失敗。
